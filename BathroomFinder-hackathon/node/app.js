...
function receivedMessage(event) {
  ...
  if (messageText) {
    ...
    var myObject = JSON.parse(JSON.stringify(messageAttachments));
    var myLocation = JSON.parse(JSON.stringify(myObject[0].payload));
    console.log("My location: %d,%d", myLocation.coordinates.lat,myLocation.coordinates.long);
    sendGenericMessage(senderID,myLocation);
  }
}
...
function sendHiMessage(recipientId) {
  var messageData = {
    ...
    message: {
      text: `
Thank you for signing up with PottyTime - The Bathroom Finder! Our Bot will let you find any near by bathrooms.
      `
    }
  };
        ...
        attachment:{
          ...
          payload:{
            ...
            buttons:[
              {
                type:"web_url",
                url:"https://www.facebook.com/pottyTimeTechnicolor/",
                title:"URL Button",
 ...
 *
 */
function sendGenericMessage(recipientId,location) {
  var createURL = "http://fb-map-server.herokuapp.com/?latlng=";
  createURL = createURL.concat(location.coordinates.lat,",",location.coordinates.long);
  console.log(createURL);
  console.log(location.coordinates.lat,location.coordinates.long);
  var messageData = {
    recipient: {
      id: recipientId
    },
    message: {
      attachment: {
        ...
        payload: {
          ...
          elements: [
            {
              title: "Bathrooms near you",
              item_url:
              createURL,
              image_url:
                "http://www.androidguys.com/wp-content/uploads/2015/12/Google-Maps.jpg",
              buttons: [
                {
                  type: "web_url",
                  url:
                  createURL,
                  title: "Open nearby locations"
                }
              ]
            }
          ]
...
function sendSelectionMessage(recipientId) {
  var messageData = {
    recipient: {
      id: recipientId
    },
      ...
      quick_replies: [
        {
          content_type: "text",
          title: "Find Bathrooms",
          payload: "DEVELOPER_DEFINED_PAYLOAD_FOR_PICKING_BATHROOM"
        },
        {
          content_type: "text",
          title: "Add Bathrooms",
          payload: "DEVELOPER_DEFINED_PAYLOAD_FOR_ADDING_BATHROOM"
        }
      ]
    }
  };
